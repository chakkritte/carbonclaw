...
class InstructionTierManager:
    """Manages hierarchical instruction files (CARBONCLAW.md, MEMORY.md)."""

    def __init__(self, root_dir: Optional[Path] = None) -> None:
        self.root_dir = root_dir or Path.cwd()
        self.user_dir = Path(platformdirs.user_config_dir("carbonclaw"))
        self.global_memory_file = self.user_dir / "CARBONCLAW.md"
    ...
    def get_project_instructions(self) -> str:
        """Read project-wide instructions (CARBONCLAW.md in root)."""
        project_file = self.root_dir / "CARBONCLAW.md"
    ...
    def get_scoped_instructions(self, current_dir: Optional[Path] = None) -> str:
        ...
        # Find the nearest CARBONCLAW.md going up until root_dir
        curr = search_dir
        while curr.exists() and curr.is_relative_to(self.root_dir):
            scoped_file = curr / "CARBONCLAW.md"
            if scoped_file.exists() and scoped_file != (self.root_dir / "CARBONCLAW.md"):
