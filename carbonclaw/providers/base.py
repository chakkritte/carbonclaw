...
from carbonclaw.core.models import Message, StreamChunk, ToolCall, ToolCallChunk
...
def _message_to_openai(msg: Message) -> dict[str, Any]:
    """Convert a CarbonClaw Message to OpenAI chat completion format."""
    if msg.role == "tool":
        return {
            "role": "tool",
...
def _message_to_anthropic(msg: Message) -> dict[str, Any]:
    """Convert a CarbonClaw Message to Anthropic format."""
    if msg.role == "tool":
        return {
            "role": "user",
            "content": [
...
class ToolAccumulator:
    ...
    def ingest(self, tool_call_chunks: list[Any]) -> list[StreamChunk]:
        """Process raw provider tool call chunks and emit CarbonClaw StreamChunks."""
