...
class CarbonClawPlugin:
    """Base class for CarbonClaw plugins.

    Plugins are discovered via Python entry points under the group
    ``carbonclaw.plugins``. Each entry point should return a plugin instance
...
class PluginRegistry:
    ...
    def __init__(self) -> None:
        self._plugins: dict[str, CarbonClawPlugin] = {}
    ...
    def discover(self) -> None:
        """Discover plugins via ``carbonclaw.plugins`` entry points."""
        eps: Any
        try:
            ...
            eps = importlib.metadata.entry_points(group="carbonclaw.plugins")
        except (AttributeError, TypeError):
            # Fallback for older metadata API
            all_eps = importlib.metadata.entry_points()
            if hasattr(all_eps, "select"):
                eps = all_eps.select(group="carbonclaw.plugins")
            else:
                eps = getattr(all_eps, "get", lambda k, default: [])("carbonclaw.plugins", [])

        for ep in eps:
            try:
                factory = ep.load()
                plugin = factory() if callable(factory) else factory
                if isinstance(plugin, CarbonClawPlugin):
                    self.load(plugin)
    ...
    def load(self, plugin: CarbonClawPlugin) -> None:
