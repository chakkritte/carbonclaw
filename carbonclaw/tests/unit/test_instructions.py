...
def test_instruction_tier_manager_aggregation(temp_workspace, monkeypatch):
    ...
    monkeypatch.setattr("platformdirs.user_config_dir", lambda _: str(home / ".config" / "carbonclaw"))
    
    # Setup files
    global_carbonclaw = home / ".config" / "carbonclaw" / "CARBONCLAW.md"
    global_carbonclaw.parent.mkdir(parents=True, exist_ok=True)
    global_carbonclaw.write_text("Global instructions", encoding="utf-8")
    
    project_carbonclaw = root / "CARBONCLAW.md"
    project_carbonclaw.write_text("Project instructions", encoding="utf-8")
    
    scoped_carbonclaw = root / "src" / "CARBONCLAW.md"
    scoped_carbonclaw.write_text("Scoped instructions", encoding="utf-8")
